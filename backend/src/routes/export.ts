...
app.get('/json', (c) => {
  ...
  const data = {
    ...
    workCases: db.prepare('SELECT * FROM work_cases WHERE user_id = ?').all(userId).map(toCamel),
    faultCases: db.prepare('SELECT * FROM fault_cases WHERE user_id = ?').all(userId).map(toCamel),
    labs: db.prepare('SELECT * FROM labs WHERE user_id = ?').all(userId).map(toCamel),
    knowledgeCards: db.prepare('SELECT * FROM knowledge_cards WHERE user_id = ?').all(userId).map(toCamel),
    projects: db.prepare('SELECT * FROM projects WHERE user_id = ?').all(userId).map(toCamel),
    dailyLogs: db.prepare('SELECT * FROM daily_logs WHERE user_id = ?').all(userId).map(toCamel),
    timeRecords: db.prepare('SELECT * FROM time_records WHERE user_id = ?').all(userId).map(toCamel),
    timelineEvents: db.prepare('SELECT * FROM timeline_events WHERE user_id = ?').all(userId).map(toCamel),
  }
...
app.post('/import', async (c) => {
  ...
  // 校验导出格式
  const requiredKeys = [
    'workCases', 'faultCases', 'labs', 'knowledgeCards',
  ...
  const tableMap: Record<string, { table: string; columns: string[] }> = {
    ...
    labs: {
      table: 'labs',
      columns: ['title', 'goal', 'environment', 'steps', 'result', 'pitfalls', 'summary'],
    },
    knowledgeCards: {
  ...
  for (const key of requiredKeys) {
    const { table, columns } = tableMap[key]
