...
app.get('/', (c) => {
  ...
  if (search) {
    where += ' AND (content LIKE ? OR asset_type LIKE ? OR gain LIKE ? OR problem LIKE ?)'
    const like = `%${search}%`
...
app.put('/:id', async (c) => {
  ...
  db.prepare(`UPDATE daily_logs SET ${sets} WHERE id = ? AND user_id = ?`).run(...values, id, userId)
  const item = db.prepare('SELECT * FROM daily_logs WHERE id = ?').get(id)
  return c.json(ok(toCamel(item)))
})
...
app.delete('/:id', (c) => {
  const userId = c.get('userId') as number
  const id = Number(c.req.param('id'))
  const existing = db.prepare('SELECT * FROM daily_logs WHERE id = ? AND user_id = ? AND deleted_at IS NULL').get(id, userId)
  if (!existing) return c.json(fail('记录不存在'), 404)
  db.prepare("UPDATE daily_logs SET deleted_at = datetime('now', '+8 hours') WHERE id = ? AND user_id = ?").run(id, userId)
...
app.post('/:id/restore', (c) => {
  const userId = c.get('userId') as number
  const id = Number(c.req.param('id'))
  const existing = db.prepare('SELECT * FROM daily_logs WHERE id = ? AND user_id = ? AND deleted_at IS NOT NULL').get(id, userId)
  if (!existing) return c.json(fail('记录不存在或未被删除'), 404)
  db.prepare('UPDATE daily_logs SET deleted_at = NULL WHERE id = ? AND user_id = ?').run(id, userId)
  const item = db.prepare('SELECT * FROM daily_logs WHERE id = ?').get(id)
