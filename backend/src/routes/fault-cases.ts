...
app.get('/tags', (c) => {
  try {
    ...
    const rows = db.prepare('SELECT tags FROM fault_cases WHERE user_id = ? AND deleted_at IS NULL').all(userId) as any[]
    ...
    console.error('Fault cases tags error:', err)
...
app.get('/', (c) => {
  try {
    ...
    if (search) {
      where += ' AND (title LIKE ? OR symptom LIKE ? OR root_cause LIKE ? OR solution LIKE ?)'
      const like = `%${search}%`
    ...
    console.error('Fault cases list error:', err)
...
app.delete('/:id', (c) => {
  const userId = c.get('userId') as number
  const id = Number(c.req.param('id'))
  const existing = db.prepare('SELECT * FROM fault_cases WHERE id = ? AND user_id = ? AND deleted_at IS NULL').get(id, userId)
  if (!existing) return c.json(fail('记录不存在'), 404)
  db.prepare("UPDATE fault_cases SET deleted_at = datetime('now', '+8 hours') WHERE id = ? AND user_id = ?").run(id, userId)
...
app.post('/:id/restore', (c) => {
  const userId = c.get('userId') as number
  const id = Number(c.req.param('id'))
  const existing = db.prepare('SELECT * FROM fault_cases WHERE id = ? AND user_id = ? AND deleted_at IS NOT NULL').get(id, userId)
  if (!existing) return c.json(fail('记录不存在或未被删除'), 404)
  db.prepare('UPDATE fault_cases SET deleted_at = NULL WHERE id = ? AND user_id = ?').run(id, userId)
  const item = db.prepare('SELECT * FROM fault_cases WHERE id = ?').get(id)
